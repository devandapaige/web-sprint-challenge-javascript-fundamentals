...
*/
const displayNames = [];
zooAnimals.forEach((animalObj) => {
  displayNames.push({
    Name: animalObj.animal_name,
    Scientific: animalObj.scientific_name,
  });
});
...
const lowCaseAnimalNames = [
  zooAnimals.map((animalObj) => {
    let lowerCase = animalObj.animal_name;
...
const lowPopulationAnimals = [
  zooAnimals.filter((animalObj) => {
    if (animalObj.population < 5) {
      return true;
    } else {
      return false;
    }
...
*/
let populationTotal = zooAnimals.reduce((sum, animalObj) => {
  return sum + animalObj.population;
}, 0);
console.log(
  `Population Total of Zoo animals (should === 56): ${populationTotal}`
);
